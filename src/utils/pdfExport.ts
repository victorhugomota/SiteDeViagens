...
export function exportTripToPdf(trip: Trip): void {
  ...
  const htmlContent = `
    ...
    <body>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td><strong style="color: #0891b2;">${formatCurrency(trip.accommodation?.totalCost || 0)}</strong></td>
          </tr>
          <tr>
            ...
            <td><strong style="color: #0891b2;">${formatCurrency(trip.transport?.calculatedFuelCost || 0)}</strong></td>
          </tr>
          <tr>
            <td><strong>Pedágios</strong></td>
            <td>Praças de pedágio na rota</td>
            <td>Estimado para Ida e Volta</td>
            <td><strong style="color: #0891b2;">${formatCurrency(trip.transport?.tollCost || 0)}</strong></td>
          </tr>
          ...
          <tr>
            ...
            <td><strong style="color: #0891b2;">${formatCurrency(trip.carRental.totalCost)}</strong></td>
          </tr>
          ` : ''}
        </tbody>
      </table>
        ...
        <tbody>
          ${extraItems.map((item, idx) => `
            <tr>
              ...
              <td style="text-align: right;"><strong style="color: #0891b2;">${formatCurrency(item.value)}</strong></td>
            </tr>
          `).join('')}
        </tbody>
      </table>
      ...
      <p style="background: #f8fafc; padding: 10px; border-radius: 8px; border: 1px solid #e2e8f0; margin-bottom: 16px; color: #334155;">${trip.notes}</p>
      ` : ''}
      ...
      <div class="page-break">
        ...
        <p style="font-size: 11px; color: #64748b; margin-bottom: 12px;">
          Opções sugeridas na região de <strong>${trip.destinationAddress}</strong>. Clique em qualquer local para pesquisar detalhes no Google:
        </p>
            ...
            ${restaurants.map(r => `
              <div class="rec-item">
                <a href="${getGoogleSearchUrl(r.name, r.address)}" target="_blank" rel="noopener noreferrer" class="rec-link" title="Abrir ${r.name} no Google">
                  <div><strong style="color:#0891b2;">${r.name} ↗</strong><br><span style="color:#64748b;">${r.distance} • ${r.address}</span></div>
                  <span style="font-weight:bold; color:#e11d48; margin-left: 6px;">★ ${r.rating}</span>
                </a>
              </div>
            `).join('')}
          </div>
            ...
            ${cafes.map(c => `
              <div class="rec-item">
                <a href="${getGoogleSearchUrl(c.name, c.address)}" target="_blank" rel="noopener noreferrer" class="rec-link" title="Abrir ${c.name} no Google">
                  <div><strong style="color:#0891b2;">${c.name} ↗</strong><br><span style="color:#64748b;">${c.distance} • ${c.address}</span></div>
                  <span style="font-weight:bold; color:#d97706; margin-left: 6px;">★ ${c.rating}</span>
                </a>
              </div>
            `).join('')}
          </div>
            ...
            ${hotels.map(h => `
              <div class="rec-item">
                <a href="${getGoogleSearchUrl(h.name, h.address)}" target="_blank" rel="noopener noreferrer" class="rec-link" title="Abrir ${h.name} no Google">
                  <div><strong style="color:#0891b2;">${h.name} ↗</strong><br><span style="color:#64748b;">${h.distance} • ${h.address}</span></div>
                  <span style="font-weight:bold; color:#4f46e5; margin-left: 6px;">★ ${h.rating}</span>
                </a>
              </div>
            `).join('')}
          </div>
            ...
            ${attractions.map(a => `
              <div class="rec-item">
                <a href="${getGoogleSearchUrl(a.name, a.address)}" target="_blank" rel="noopener noreferrer" class="rec-link" title="Abrir ${a.name} no Google">
                  <div><strong style="color:#0891b2;">${a.name} ↗</strong><br><span style="color:#64748b;">${a.distance} • ${a.address}</span></div>
                  <span style="font-weight:bold; color:#0891b2; margin-left: 6px;">★ ${a.rating}</span>
                </a>
              </div>
            `).join('')}
          </div>
